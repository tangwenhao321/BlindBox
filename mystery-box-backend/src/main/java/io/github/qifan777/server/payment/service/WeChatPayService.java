...
public class WeChatPayService {
    ...
    public WxPayUnifiedOrderV3Request create(WeChatPayModel weChatPayModel) {
        ...
        payer.setOpenid(userWeChatRepository.sql().createQuery(t)
                ...
                .select(t)
                .fetchOptional()
                .orElseThrow(() -> new BusinessException(ResultCode.NotFindError, "当前账号未绑定微信支付身份, 请先注册绑定")).openId());
        wxPayUnifiedOrderV3Request.setPayer(payer)
                .setAmount(amount)
                // 回调地址
                .setNotifyUrl(resolveNotifyUrl(weChatPayModel.getNotifyUrl()))
                // 微信支付 AppId（JSAPI）
    ...
    private String resolveNotifyUrl(String path) {
        String base = wxPayPropertiesExtension.getNotifyUrl();
        String suffix = path == null ? "" : path.trim();
        if (!StringUtils.hasText(base)) {
            return suffix;
        }
        if (!StringUtils.hasText(suffix) || base.endsWith(suffix)) {
            return base;
        }
        if (base.endsWith("/")) {
            return base + suffix.replaceFirst("^/", "");
        }
