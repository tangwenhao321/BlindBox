...
import type { CouponBoxRelDto } from '@/apis/__generated/model/dto'
...
type CouponBoxRelScope = Scope<
  CouponBoxRelDto['CouponBoxRelRepository/COMPLEX_FETCHER_FOR_ADMIN']
>
const couponBoxRelTableHelper = inject(
  'couponBoxRelTableHelper',
  useTableHelper(api.couponBoxRelForAdminController.query, api.couponBoxRelForAdminController, {})
)
const {
  loadTableData,
  handleSortChange,
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.couponBoxRelForAdminController.delete({ body: ids }).then((res) => {
  ...
  <div>
    ...
    >
      ...
      <el-table-column
        ...
        width="140"
      >
        <template v-slot:default="{ row }: CouponBoxRelScope">
          {{ row.coupon.name }}
        </template>
      </el-table-column>
      <el-table-column
