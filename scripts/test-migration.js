...
async function main() {
  ...
  try {
    ...
    await exec(
      conn,
      "docker exec ehpay-mysql mysql -uroot -p" + _dbPass + " mystery_box_test < /opt/mystery-box-test/src/mystery-box-backend/src/main/resources/db/migration/V20260500_01__core_schema_baseline.sql",
    );
    await upload(conn, sqlPath, "/tmp/test-migration.sql");
    await exec(
      conn,
      "docker exec -i ehpay-mysql mysql -uroot -p" + _dbPass + " mystery_box_test < /tmp/test-migration.sql",
    );
