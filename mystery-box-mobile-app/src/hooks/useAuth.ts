...
import { loginByPhone, registerByPhone } from "../services/authService";
...
export function useAuth() {
  ...
  const login = async () => {
    setSubmitting(true);
    try {
      const nextToken = await loginByPhone(phone, password);
      setToken(nextToken);
  ...
  const register = async (inviteCode?: string, smsCode?: string) => {
    ...
    try {
      ...
      const nextToken = await registerByPhone(phone, password, code, inviteCode);
      setToken(nextToken);
      await writeToken(nextToken);
      await initRevealStorageNamespace(nextToken.slice(0, 16));
      return nextToken;
