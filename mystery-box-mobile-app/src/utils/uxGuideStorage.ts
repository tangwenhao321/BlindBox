...
const SIMPLE_REVEAL_TIP_KEY = "ux_guide_simple_reveal_tip_v1";
...
/** First-time tip pointing at simple/performance reveal — does not change preference. */
export async function shouldShowSimpleRevealTip(): Promise<boolean> {
  return (await AsyncStorage.getItem(SIMPLE_REVEAL_TIP_KEY)) !== "1";
}

export async function dismissSimpleRevealTip(): Promise<void> {
  await AsyncStorage.setItem(SIMPLE_REVEAL_TIP_KEY, "1");
}
